<html>
	<head>
		<link type="text/css" rel="stylesheet" href="CSS/ScheduleGroupSession.css" />
		<title> Schedule Group Sessions </title>
	</head>
				...
				<div id="center">
					<?php
						...
						function main()
						{
							...
							$scheduler = new IndividualAppt($month,$day,$year);
						...
						class IndividualAppt
						{
							...
							// constructor
							function IndividualAppt($month,$day,$year)
							{
								$this->m_month = $month;
								$this->m_day = $day;
								$this->m_year = $year;
							...
							function isAvailable($beginTime)
							{
								...
								$sql = "SELECT `sessionID` FROM `GroupAdvisingSessions`";
								...
								if($this->isDatePlausible())
								{
									if($this->isDateInFuture($beginTime))
									{
										if($this->isAvailable($beginTime))
										{
											...
											$sql = "INSERT INTO `GroupAdvisingSessions`(`day`, `month`, `year`,";
											$sql .= "`beginHour`, `beginMin`, `endHour`, `endMin`)";
